...
opkg install jq curl coreutils-base64 || true
...
echo "[4/5] Request WARP config"
response=$(curl -s --max-time 60 https://warp-gen.vercel.app/generate-config)
[ -z "$response" ] && { echo "Failed to get WARP config"; exit 1; }
...
echo "[5/5] Configure interface awg10"
...
uci set network.@amneziawg[-1].description="$PEER"
...
ifdown awg10 2>/dev/null || true
sleep 2
ifup awg10

echo "✅ awg10 configured and started"
